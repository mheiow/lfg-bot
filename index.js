...
async function startHostWizard(ix, cfg){
  ...
  const i6a = await msg6a.awaitMessageComponent({ componentType:2, time:120_000 }).catch(()=>null);
  if (!i6a) return void ix.followUp({ content:'Timed out.', ephemeral:true });
  s.micRequired = i6a.customId.endsWith(':on');
  await i6a.update({ components:[] });
  ...
  const row6b = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId('ps:4').setLabel('+4').setStyle(ButtonStyle.Secondary)
  );
  ...
  const i6b = await msg6b.awaitMessageComponent({ componentType:2, time:120_000 }).catch(()=>null);
  if (!i6b) return void ix.followUp({ content:'Timed out.', ephemeral:true });
  s.playersNeeded = Math.min(4, Math.max(1, parseInt(i6b.customId.split(':')[1])));
  await i6b.update({ components:[] });
